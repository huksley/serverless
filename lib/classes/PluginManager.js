...
class PluginManager {
  ...
  setCliOptions(options) {
    // globally allow setting the --config option on all commands
    if ('config' in options) {
      this.configFilePath = path.resolve(options.config);
    } else if ('c' in options) {
      this.configFilePath = path.resolve(options.c);
    }

    if (this.configFilePath && !fs.existsSync(this.configFilePath)) {
      throw new this.serverless.classes.Error('config file not found');
    }
  ...
  validateServerlessConfigDependency(command) {
    if ('configDependent' in command && command.configDependent) {
      return getServerlessConfigFile(this.configFilePath).then((serverlessConfigFile) => {
  ...
  updateAutocompleteCacheFile() {
    ...
    return getServerlessConfigFile(this.configFilePath || this.serverless.config.servicePath)
      .then((serverlessConfigFile) => {
