...
describe('PluginManager', () => {
  ...
  describe('#setCliOptions()', () => {
    ...
    it('should recognise and parse -c and --config flags', () => {
      const serverlessFilePath = path.join(tmpDirPath, 'serverless-test.yaml');
      const options = { c: serverlessFilePath };

      writeFileSync(serverlessFilePath, 'service: my-yaml-service');
      pluginManager.setCliOptions(options);
      expect(pluginManager.configFilePath).to.deep.equal(serverlessFilePath);
    });

    it('should throw an error if -c or --config file does not exist', () => {
      const options = { c: '/tmp/idonotexist.yml' };

      expect(pluginManager.setCliOptions.bind(pluginManager, options))
